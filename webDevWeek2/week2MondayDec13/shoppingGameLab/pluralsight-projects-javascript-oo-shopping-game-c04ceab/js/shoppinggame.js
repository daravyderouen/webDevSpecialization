...
const dateDiff = (date1, date2) => {
    return Math.ceil(Math.abs(date1-date2) /( 1000 * 60 * 60 * 24)) // 1000 60 60 24, turns it into seconds, minutes, hours and then days
    //the Math.ceil is rounding up the number
};
...
const calculatePoints = (prod, tBill) => {
    ...
    player.score = player.score + pointsToBill + findPointsForExpDate;
...
function init(data) {
    ...
    const shop = (prodList, tBill, lastProd) => {
        ...
        rl.question(`You can buy - ${productDetails}.\n Do you want to buy this item <Y/N>? `.yellow, function (option) {
            ...
            const regexNo = new RegExp('y', 'i');; // Use the RegExp built-in object type here as appropriate
